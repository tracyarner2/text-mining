...
2.  How does the sentiment analysis address research questions?
    ...
    -   The sentiment analysis looks at sentiment of tweets over time with a line graph of positive, negative, and neutral tweets as well as a word cloud. The tweets for the sentiment analysis were taken from the account of a Bangladeshi businessman and the data dictionaries that they used to detect affect were custom text files. The purpose of the paper (i.e., research question) was to explore whether or not sentiment analysis could be useful in analyzing tweets as well as what information could be gleaned from generating a word cloud from tweets overtime. 
...
The data that would need to be collected would be tweets that include #hashtags pertaining to both the CCSS standards and states such as #AZ, #Arizona. 

2.  For what reason would text data need to be collected in order to address this question?

The text of the tweet would include language that would specify positive or negative valence as people voice opinions on the benefit or detriment of adoption of Common Core State Standards.  

3.  Explain the analytical level at which these text data would need to be collected and analyzed.

These data would be collected and sorted at the tweet level, but would be analyzed at the word level to apply the sentiment dictionaries.  
